...
void copiarVetor(automovel *destino, automovel *origem, int qtdAutomoveis);
...
void inserirNovaPessoa(pessoa novaPessoa){
    ...
    /*strcpy(cadastros[0].nome, novaPessoa.nome);
    strcpy(cadastros[0].cpf, novaPessoa.cpf);
    cadastros[0].qtdAuto = novaPessoa.qtdAuto;
    copiarVetor(cadastros[0].automoveis, novaPessoa.automoveis, novaPessoa.qtdAuto);*/
    ...
    if(pArq == NULL){
        ...
        while(fread(&auxArquivo,sizeof(auxArquivo),1,pArq)){
            /*strcpy(cadastros[aux].nome, auxArquivo.nome);
            strcpy(cadastros[aux].cpf, auxArquivo.cpf);
            cadastros[aux].qtdAuto = auxArquivo.qtdAuto;
            copiarVetor(cadastros[aux].automoveis, auxArquivo.automoveis, auxArquivo.qtdAuto);*/
...
void copiarVetor(automovel *destino, automovel *origem, int qtdAutomoveis){
    int aux;
    for(aux = 0; aux < qtdAutomoveis; aux++){
        strcpy(destino[aux].marca, origem[aux].marca);
        strcpy(destino[aux].placa, origem[aux].placa);
        strcpy(destino[aux].modelo, origem[aux].modelo);
        strcpy(destino[aux].cor, origem[aux].cor);
    }
}
